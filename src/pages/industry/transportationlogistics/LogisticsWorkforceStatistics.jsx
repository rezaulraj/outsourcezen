...
const LogisticsWorkforceStatistics = () => {
  const sectionRef = useRef(null);

  useEffect(() => {
    const ctx = gsap.context(() => {
      gsap.from(".logi-stat-word", {
        y: 50,
        opacity: 0,
        rotateX: 55,
        duration: 0.9,
        ...
        <div className="mx-auto mb-14 max-w-3xl text-center">
          <div className="mb-5 inline-flex items-center gap-3 rounded-full border border-black/10 bg-[#FFF9E6] px-5 py-2">
            ...
            <span className="text-sm font-semibold text-black">
              Logistics Workforce Statistics
            </span>
          </div>

          <h2 className="text-4xl font-normal tracking-[-0.045em] text-black sm:text-5xl lg:text-6xl">
            {["Numbers", "behind", "reliable", "logistics", "movement"].map(
              (word) => (
                <span key={word} className="inline-block overflow-hidden px-1">
                  <span className="logi-stat-word inline-block">{word}</span>
                </span>
              ),
            )}
          </h2>
          ...
          <p className="mx-auto mt-5 max-w-xl text-sm leading-6 text-black/70">
            Reliable logistics recruitment support for drivers, warehouse teams,
            dispatch staff, delivery operations and supply chain workforce
            needs.
          </p>
        </div>
...
const StatisticCard = ({ item }) => {
  ...
  return (
    <article className="logi-stat-card group relative overflow-hidden rounded-[32px] border border-black/10 bg-[#FFF9E6] p-7 transition-all duration-300 hover:-translate-y-2 hover:border-black/20">
      <div
...
export default LogisticsWorkforceStatistics;
