...
const HeroWorkforceSourcing = () => {
  ...
  useEffect(() => {
    ...
    // real candidate headshots that rotate through rings over time
    const faceUrls = [
    ...
    // Even rectangular grid that always fits fully inside the canvas.
    const COLS = 6;
    const ROWS = 6;

    // Content "library" that any ring slot can be assigned: a face,
    // an icon, or nothing (an empty progress ring). Each slot cycles
    // through a personal sequence of these on its own clock, so the
    // grid is never static — things keep appearing and disappearing
    // independently across the whole board.
    ...
    for (let row = 0; row < ROWS; row++) {
      for (let col = 0; col < COLS; col++) {
        ringSlots.push({
          ...
          // Each slot has its own content cycle length and offset so
          // transitions across the grid feel organic, not synchronized.
          cycleLen: 2.8 + ((idx * 0.37) % 1.6), // seconds per content swap
    ...
    // Returns the content for a slot at a given time, plus a 0-1
    // "fade" value used to crossfade between the outgoing and
    // incoming content smoothly instead of popping.
    const FADE_PORTION = 0.28; // fraction of each cycle spent fading
  ...
  return (
    <section ref={heroRef} className="font-sans relative overflow-hidden pt-26">
      <div className="relative z-10 container mx-auto bg-[#CFEFC4] rounded-xl px-4 pb-12 pt-26 sm:px-6 lg:px-8 lg:pb-16 lg:pt-20">
        <div className="grid items-center gap-10 lg:grid-cols-2">
          <div>
            <span className="ws-reveal inline-flex items-center gap-2 rounded-full border border-black/15 bg-black/[0.04] px-4 py-1.5 text-xs font-semibold uppercase tracking-[0.14em] text-black/70">
              <Sparkles size={14} strokeWidth={2.2} />
              Workforce Sourcing
            </span>

            <h1 className="mt-5 text-[2.6rem] font-semibold leading-[1.08] tracking-[-0.03em] text-black sm:text-[3.4rem] lg:text-[3.7rem]">
              {["Connecting", "businesses", "with", "the", "right"].map(
                (word, index) => (
                  <span
                    key={index}
                    className="mr-3 inline-block overflow-hidden pb-2 last:mr-0"
                  >
                    <span className="ws-word inline-block">{word}</span>
                  </span>
                ),
              )}
              <br />
              <span className="relative mt-1 inline-block overflow-visible">
                <span className="mr-3 inline-block overflow-hidden pb-2">
                  <span className="ws-word inline-block">talent,</span>
                </span>
                <span className="relative inline-block overflow-hidden pb-2">
                  <span className="ws-word inline-block text-[#1f7a2e]">
                    faster.
                  </span>
                </span>
                <svg
                  className="ws-underline pointer-events-none absolute -bottom-2 left-0 h-4 w-full"
                  viewBox="0 0 320 24"
            ...
            <p className="ws-reveal mt-7 max-w-lg text-base leading-7 text-black/75">
              We identify, attract, and deliver highly qualified professionals
              tailored to your business needs. Through strategic sourcing,
              global talent networks, and rigorous screening, every candidate we
              put forward fits your skill requirements, your culture, and your
              long-term goals.
            </p>

            <div className="ws-reveal mt-8 flex flex-col gap-3 sm:flex-row sm:items-center">
              <a
                href="/contact"
          ...
          <div className="ws-reveal relative h-[380px] w-full sm:h-[460px] lg:h-[560px]">
...
export default HeroWorkforceSourcing;
